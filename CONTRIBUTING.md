...
✨ Add a map editor flood-fill tool
📝 Document the cart PNG chunk layout
```

A few rules to keep the prefixes consistent:

* The emoji **must** come from the curated [gitmoji](https://gitmoji.dev/) set, copied verbatim —
  including any [U+FE0F variation selector](https://emojipedia.org/variation-selector-16) (e.g.
  `♻️`, `⚡️`). Do not type the emoji from memory or strip the variation selector.
* Use exactly **one space** between the emoji and the rest of the subject.

The easiest way to get this right is to let a tool insert the emoji for you. We use
[gimoji](https://github.com/zeenix/gimoji): run `gimoji --init` once to install its
`prepare-commit-msg` hook, and it will prompt you to pick an emoji from the set on every commit. You
can also browse the set in the web picker at <https://zeenix.github.io/gimoji/>.
...
* agree to providing the content under the project's [GPL-3.0-or-later](LICENSE) license.
...
If you intend to contribute often or think that's very likely, we recommend you setup the following
git hooks:
