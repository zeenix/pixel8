...
`pixel8-player` is the third frontend over the console runtime: a pure-Rust
...
```text
pixel8-player cart.png      play one cart
pixel8-player /path/to/dir  cart picker over a directory
pixel8-player               picker over the current directory
```
...
raw evdev button indices via `PIXEL8_SELECT` / `PIXEL8_START`.

## Installing on the device (ArkOS / ROCKNIX style)

1. Build the bundle (see below) or grab the `pixel8-handheld` artifact
   from CI. It contains:

   ```text
   Pixel8.sh                 launcher (picks the right binary)
   pixel8/
     pixel8-player.armhf      32-bit ARM build
     pixel8-player.aarch64    64-bit ARM build
...
2. Copy `Pixel8.sh` and the `pixel8/` folder into the ports directory
...
3. Drop cart `.png`s into `pixel8/carts/`.
4. Refresh the games list (or reboot); launch **Pixel8** from Ports.
...
log at `pixel8/log.txt` (next to the binary) says why.
...
the same friendly Pixel8 error screen as everywhere else.
...
- The screen is letterboxed to square (640x480 -> 480x480 on the
  RGB10S) with nearest-neighbor scaling. Set `PIXEL8_ROTATE=90`,
  `180`, or `270` to rotate the output, and `PIXEL8_DRM_CARD` to
...
- Audio is the same 4-channel synth at 44.1 kHz written straight to
  ...
  Set `PIXEL8_NOAUDIO=1` to disable audio entirely.
