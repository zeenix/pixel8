# The Pixel8 PNG cartridge format (version 1)

A Pixel8 cart is a valid PNG image. The picture is the cartridge: body,
color stripes, a 128x128 label (the cart's captured screenshot, or the
built-in Pixel8 art), title and author. The game itself rides inside a
private ancillary chunk, so any image tool sees a normal 160x205 PNG
and Pixel8 sees a game.
...
pxRt                 the Pixel8 payload chunk
IEND
```

`pxRt` chunk-type properties, per the PNG spec: ancillary (`p`),
private (`x`), reserved bit valid (`R`), safe-to-copy (`t`) — image
editors that don't know about Pixel8 are allowed to pass it through.
...
- **editable cart** — `source: Some(...)`. `pixel8 extract cart.png dir`
...
On load, Pixel8 checks: PNG signature, `pxRt` CRC, magic, exact format
