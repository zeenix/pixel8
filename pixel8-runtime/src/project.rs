...
//!   assets.pixel8    # sprites/map/sfx/music/metadata (postcard, versioned)
...
const ASSETS_MAGIC: &[u8; 7] = b"PIXEL8A";
/// `assets.pixel8` body format version, so a future (post-release) format change
...
"#
    ...
    pub fn load(dir: &Path) -> Result<Self> {
        ...
        let assets = match fs::read(dir.join("assets.pixel8")) {
    ...
    pub fn save(&self) -> Result<()> {
        ...
        fs::write(self.dir.join("assets.pixel8"), encode_assets(&self.assets)?)?;
        Ok(())
    }
...
pub fn decode_assets(bytes: &[u8]) -> Result<Assets> {
    let body = bytes
        ...
        .ok_or_else(|| anyhow!("assets.pixel8 has an unknown header"))?;
    let (&version, body) = body
        .split_first()
        .ok_or_else(|| anyhow!("assets.pixel8 header is truncated"))?;
    if version != ASSETS_VERSION {
        bail!(
            "assets.pixel8 is format version {version}, but this Pixel8 needs \
...
mod tests {
    ...
    fn example_assets_load_in_the_current_format() {
        ...
        for dir in [
            ...
            let path = root.join(dir).join("assets.pixel8");
            let bytes =
    ...
    fn assets_header_is_checked() {
        assert!(decode_assets(b"NOTPIXEL8").is_err());
        // A file from a different format version is rejected, not mis-parsed.
        assert!(decode_assets(b"PIXEL8A\x02anything").is_err());
        let bytes = encode_assets(&Assets::default()).unwrap();
        assert_eq!(bytes[7], ASSETS_VERSION, "version byte follows the magic");
