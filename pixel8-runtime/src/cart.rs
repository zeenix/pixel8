...
//!
//! A Pixel8 cart is a real PNG image — the picture *is* the cartridge,
//! complete with label art — with the game embedded in a private ancillary
//! chunk. Any image viewer shows the cart; Pixel8 plays it.
//!
...
/// `pxRt` without decompressing anything — used by cart pickers to
...
pub fn default_label() -> Vec<u8> {
    ...
    // Big Pixel8 wordmark via the scaled built-in font.
...
mod tests {
    ...
    fn corrupted_payload_is_rejected() {
        ...
        // Flip a byte inside the pxRt chunk body.
        let pos = png
            .windows(4)
    ...
    fn future_version_is_rejected() {
        let cart = test_cart();
        let mut png = encode(&cart).unwrap();
        // Bump the version field (magic is 6 bytes after the chunk type+4 len... find it).
        let pos = png
            .windows(10)
