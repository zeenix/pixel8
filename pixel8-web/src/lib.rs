//! The Pixel8 browser player.
//!
//! This crate is the desktop console's engine room — `pixel8-runtime`,
...
//!
//! ```text
//! pixel8_web_upload_begin(len) -> ptr   stage a cart.png upload
//! pixel8_web_load() -> 0|1              parse + boot the staged cart
//! pixel8_web_fps() -> 30|60             the cart's logical frame rate
//! pixel8_web_error_ptr/len()            UTF-8 error text after a failure
//! pixel8_web_set_button(b, down)        buttons 0..6, like the ABI
//! pixel8_web_tick() -> 0|1              one logical frame; 1 = cart error
//! pixel8_web_fb_ptr() -> ptr            128*128*4 RGBA, valid after tick
//! pixel8_web_audio_render(n) -> n       render mono f32 samples @ 44100
//! pixel8_web_audio_ptr() -> ptr         the rendered samples
//! ```
...
/// Maximum samples per `pixel8_web_audio_render` call.
...
fn error_screen(message: &str) -> Framebuffer {
    let mut fb = pixel8_runtime::ui::error_screen(message);
...
#[no_mangle]
pub extern "C" fn pixel8_web_upload_begin(len: u32) -> *mut u8 {
...
/// text is available via `pixel8_web_error_ptr/len`.
#[no_mangle]
pub extern "C" fn pixel8_web_load() -> i32 {
    get(&ERROR).clear();
...
#[no_mangle]
pub extern "C" fn pixel8_web_error_ptr() -> *const u8 {
    get(&ERROR).as_ptr()
}

#[no_mangle]
pub extern "C" fn pixel8_web_error_len() -> u32 {
    get(&ERROR).len() as u32
}

/// The loaded cart's logical frame rate (30 or 60). The page reads this
/// after a successful `pixel8_web_load` to set its fixed timestep.
#[no_mangle]
pub extern "C" fn pixel8_web_fps() -> u32 {
    match get(&PLAYER) {
        Some(p) => p.fps(),
        None => DEFAULT_FPS,
    }
...
#[no_mangle]
pub extern "C" fn pixel8_web_set_button(b: u32, down: i32) {
...
#[no_mangle]
pub extern "C" fn pixel8_web_tick() -> i32 {
    match get(&PLAYER) {
...
#[no_mangle]
pub extern "C" fn pixel8_web_fb_ptr() -> *const u8 {
    match get(&PLAYER) {
...
#[no_mangle]
pub extern "C" fn pixel8_web_audio_render(n: u32) -> u32 {
    match get(&PLAYER) {
...
#[no_mangle]
pub extern "C" fn pixel8_web_audio_ptr() -> *const f32 {
    match get(&PLAYER) {
...
mod tests {
    ...
    fn exports_drive_the_player() {
        let png = test_cart_png(MOVER);
        let ptr = pixel8_web_upload_begin(png.len() as u32);
        unsafe { std::ptr::copy_nonoverlapping(png.as_ptr(), ptr, png.len()) };
        assert_eq!(pixel8_web_load(), 0);
        assert_eq!(pixel8_web_tick(), 0);
        assert!(!pixel8_web_fb_ptr().is_null());
        let n = pixel8_web_audio_render(512);
        assert_eq!(n, 512);
    }

    #[test]
    fn load_failure_reports_error_text() {
        let ptr = pixel8_web_upload_begin(4);
        unsafe { std::ptr::copy_nonoverlapping(b"oops".as_ptr(), ptr, 4) };
        assert_eq!(pixel8_web_load(), 1);
        assert!(pixel8_web_error_len() > 0);
    }
}
