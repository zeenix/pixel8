#!/bin/bash
# Pixel8 cart player launcher for handhelds (ArkOS / ROCKNIX / etc.),
# in the usual Ports layout:
#
#   /roms/ports/Pixel8.sh        <- this script
#   /roms/ports/pixel8/           <- pixel8-player binary
#   /roms/ports/pixel8/carts/     <- put your .png carts here
#
...
chmod +x "$DIR/"pixel8-player.* 2>/dev/null || true
...
# Override with PIXEL8_ARCH=aarch64|armhf.
echo "pixel8: $(uname -a)" >"$DIR/log.txt"
PLAYER=""
for arch in ${PIXEL8_ARCH:-aarch64 armhf}; do
  cand="$DIR/pixel8-player.$arch"
  if probe=$("$cand" --probe 2>&1); then
    echo "pixel8: probe $arch: ok ($probe)" >>"$DIR/log.txt"
    [ -z "$PLAYER" ] && PLAYER="$cand"
  else
    echo "pixel8: probe $arch: failed ($probe)" >>"$DIR/log.txt"
  fi
done
if [ -z "$PLAYER" ]; then
  echo "pixel8: no runnable binary for this device" >>"$DIR/log.txt"
  exit 1
fi
echo "pixel8: using $PLAYER" >>"$DIR/log.txt"
...
# export PIXEL8_SELECT=8
# export PIXEL8_START=9

# Full backtraces if the player panics. To rule out audio entirely while
# debugging, set PIXEL8_NOAUDIO=1 here.
export RUST_BACKTRACE=full
"$PLAYER" "$DIR/carts" >>"$DIR/log.txt" 2>&1
echo "pixel8: player exited with code $? (>128 means killed by signal N-128)" >>"$DIR/log.txt"
