...
use pixel8_runtime::{cart, project::Project};
use std::{
    path::Path,
    process::Command,
...
/// Used by the headless `pixel8 build`/`export` subcommands.
...
mod tests {
    ...
    fn wasm_size_warnings_fires_for_oversized_wasm() {
        ...
        fs::write(
            &wasm_path,
            vec![0u8; pixel8_runtime::cart::MAX_WASM_SIZE + 1],
        )
        .unwrap();
