...
struct WinHandler {
    ...
    /// Blit rotation, from `PIXEL8_ROTATE`; unset means `Rotate::None` (the desktop default).
    rotate: Rotate,
...
impl WinHandler {
    /// A handler with no window yet and rotation taken from `PIXEL8_ROTATE` (default upright).
    fn new() -> WinHandler {
        WinHandler {
            surface: None,
...
impl ApplicationHandler for WinHandler {
    fn resumed(&mut self, el: &ActiveEventLoop) {
        ...
        let window = match el.create_window(attrs) {
            Ok(w) => Rc::new(w),
            Err(e) => {
                eprintln!("pixel8-player: could not open a window: {e}");
                el.exit();
                return;
            }
        };
        let context = match Context::new(window.clone()) {
            Ok(c) => c,
            Err(e) => {
                eprintln!("pixel8-player: softbuffer context failed: {e}");
                el.exit();
                return;
            }
        };
        let surface = match Surface::new(&context, window.clone()) {
            Ok(s) => s,
            Err(e) => {
                eprintln!("pixel8-player: softbuffer surface failed: {e}");
                el.exit();
                return;
            }
        };
