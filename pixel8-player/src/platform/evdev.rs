...
/// Optional raw-index overrides (PIXEL8_SELECT / PIXEL8_START) for pads with no named buttons.
...
impl Input {
    ...
    pub fn new() -> Input {
        ...
        eprintln!("pixel8-player: {} input device(s)", devices.len());
        Input {
            devices,
            buttons: [false; 6],
            select: false,
            start: false,
            fps_edge: false,
            sel_raw: env_btn("PIXEL8_SELECT"),
            start_raw: env_btn("PIXEL8_START"),
        }
    }
