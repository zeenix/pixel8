...
use pixel8_runtime::{audio::AudioHandle, fb::Framebuffer};
...
impl KmsPlatform {
    ...
    ///
    /// The device path defaults to `/dev/dri/card0`; set `PIXEL8_DRM_CARD` to override.
...
/// Open the DRM device.
///
/// If `PIXEL8_DRM_CARD` is set, open exactly that path. Otherwise scan `/dev/dri` for `card*`
...
fn open_card() -> Result<Card> {
    if let Ok(path) = std::env::var("PIXEL8_DRM_CARD") {
    ...
    Err(anyhow!(
        ...
        Set PIXEL8_DRM_CARD to override."
    ))
}
...
/// passes this through `Rotate::from_env_or` so `PIXEL8_ROTATE` can override it.
    ...
    for (&id, &raw) in props.iter() {
        ...
        if let Value::Enum(Some(ev)) = info.value_type().convert_value(raw) {
            return match ev.name().to_str() {
                ...
                // confirm on-device — PIXEL8_ROTATE overrides if the panel is mirrored.
