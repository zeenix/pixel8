...
use pixel8_runtime::{cart, fb::Framebuffer, palette::col};
use std::path::{Path, PathBuf};

/// All Pixel8 carts in a directory, sorted by file name.
...
pub fn draw_picker(dir: &Path, carts: &[PathBuf], sel: usize, frame: u32) -> Framebuffer {
    ...
    fb.print("pixel8 carts", 2, 10, col::WHITE);
...
mod tests {
    ...
    fn scan_finds_only_carts_sorted() {
        let dir = std::env::temp_dir().join(format!("pixel8_scan_{}", std::process::id()));
