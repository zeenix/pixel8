//! Pixel8 cart player for handhelds and desktop-from-a-TTY: a pure-Rust frontend over
//! `pixel8-runtime` with a KMS/evdev/ALSA backend (no SDL). Point it at a cart or a directory.
...
use app::App;
use pixel8_runtime::audio::AudioHandle;
...
#[cfg(feature = "window")]
type AudioKeepalive = Option<pixel8_runtime::audio::AudioOutput>;
...
fn main() -> Result<()> {
    ...
    if args.first().map(String::as_str) == Some("--probe") {
        println!("pixel8-player ok arch={}", std::env::consts::ARCH);
        return Ok(());
    }
...
fn real_backend() -> Result<(Box<dyn platform::Platform>, AudioHandle, AudioKeepalive)> {
    Err(anyhow!(
        "pixel8-player was built with no display backend; enable the `window` or `kms` feature"
    ))
}
